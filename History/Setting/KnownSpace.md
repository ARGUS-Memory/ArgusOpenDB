[ARGUS Station Database](../../README.md) > [History](../README.md) > [Setting](README.md) > Known Space

# Known Space

<img src="../../assets/raptor_idle_south.png" width="96" align="right">

Reference record on the inhabited regions of the galaxy as of the current era: species, trans-stellar corporations, governing bodies, and armed factions operating across known space.

---

## Quick Reference

| | |
|---|---|
| [Era and Expansion](#era-and-expansion) | Timeline of human colonization and FTL development |
| [Vir System](#vir-system) | The frontier system where this station operates |
| [Currency](#currency) | Standard economic unit in use across known space |
| [Sapient Species](#sapient-species) | Known species encountered in the frontier |
| [Trans-Stellar Corporations](#trans-stellar-corporations) | Major corporate entities operating across systems |
| [Governing Bodies](#governing-bodies) | Sol Central, VGA, and other administrative authorities |
| [Military and Paramilitary Groups](#military-and-paramilitary-groups) | USDF, Syndicate, and armed factions in the frontier |

---

## Era and Expansion

The current era is measured in CE (Common Era). Humanity's expansion beyond the Sol system proceeded through a combination of colony ships, FTL-capable vessels, and an interstellar warpgate network connecting the major settled systems. Alpha Centauri, Tau Ceti, Sirius, and neighboring systems are linked by this network, reducing transit time between systems from decades to hours.

The discovery of **phoron** in the Gliese 229 system transformed the pace of expansion. Phoron and its interaction with **bluespace** enabled the development of on-ship FTL drives, allowing vessels to travel independently of fixed warpgate infrastructure. Systems without stable warpgates, previously impractical to colonize, became accessible. See [Phoron and Bluespace](PhoronAndBluespace.md) for the science.

---

## Vir System

Vir is a frontier system at the edge of colonized human space. The system lacks a stable warpgate, requiring FTL-capable vessels for access. This geographic isolation reduces Sol Central's practical presence to near zero and positions NanoTrasen as the de facto governing body for the system's corporate and industrial activities.

The primary inhabited body is **Sif**, a garden world orbited by the NLS Southern Cross and the larger colony station NCS Northern Star. NanoTrasen's phoron extraction and research operations in the system represent its primary foothold on the frontier.

Vir is governed by the **Vir Government Authority (VGA)**, a unitary state with full Sol Central member status. VGA maintains democratic governance independent of TSC influence, though a longstanding alliance of convenience with NanoTrasen governs corporate operations in the Kara subsystem.

See [Vir](../Locations/Vir.md) for system history and geography. See [NLS Southern Cross](../Locations/NLSSouthernCross.md) for station-specific information.

---

## Currency

The standard currency in use across most settled systems is the **Thaler**, represented by the symbol **$** (e.g., $130). The Thaler is backed and maintained by Sol Central and its allied systems. A small number of systems and regions do not recognize the Thaler as legal tender.

---

## Sapient Species

Multiple sapient species are present across known space. The following represents the species most commonly encountered in the Vir system and aboard NanoTrasen installations.

### Humans

**Origin:** Earth, Sol system.

Omnivorous bipeds and the primary driver of rapid interstellar expansion; one of the most numerous sapient species in known space. See [Humans](../../Species/Humans.md) for a full species profile.

### Skrell

**Origin:** Qerr'balak, Qerr'Vallis system.

Amphibious humanoids and humanity's oldest alien ally; the most technologically advanced species in known space. See [Skrell](../../Species/Skrell.md) for a full species profile.

### Teshari

**Origin:** Sirisai, Qerr'Vallis system.

Small cold-adapted pack predators uplifted by the skrell; organized around communal pack structures with an increasingly independent galactic presence. See [Teshari](../../Species/Teshari.md) for a full species profile.

### Unathi

**Origin:** Moghes, Uueoa-Esa system.

Large cold-blooded reptilian bipeds organized around deeply religious clan structures; humanity's second contact species and the subject of an armed conflict that continues to shape interspecies relations. See [Unathi](../../Species/Unathi.md) for a full species profile.

### Tajaran

**Origin:** Meralar, Rarkajar system.

Furred digitigrade feline-analogue bipeds uplifted into FTL capability by humanity, with a complex multi-ethnic society and an ongoing structural relationship with human employers. See [Tajaran](../../Species/Tajaran.md) for a full species profile.

### Promethean

**Origin:** Aetolus, Kataigal system.

Artificially created gelatinous humanoids derived from the Aetolian giant slime; capable of shapeshifting and rapid regeneration, and required by USDF regulation to maintain a legal guardian. See [Promethean](../../Species/Promethean.md) for a full species profile.

### Zorren

**Origin:** Virgo Prime, Virgo-Erigone system.

Small humanoids native to the Vir system, survivors of an ancient cataclysm that collapsed their interplanetary civilization to a late-medieval state. See [Zorren](../../Species/Zorren.md) for a full species profile.

### Vulpkanin

**Origin:** Altam, Vazzend binary system.

Canid bipeds with a decentralized society; notable for EM-hardened technology developed in response to Vazzend's electromagnetic flares. See [Vulpkanin](../../Species/Vulpkanin.md) for a full species profile.

### Vasilissans

**Origin:** Varilak, Antares system.

Arachnoid bipeds with four dorsal metalimbs, chitin plating, and silk production capability; native to a mineral-rich jungle world and organized under a matriarchal gem-focused religion. See [Vasilissans](../../Species/Vasilissans.md) for a full species profile.

### Diona

**Origin:** Epsilon Ursae Minoris system.

Cluster organisms that grow from nymphs into adult forms and eventually merge into vast stellar-orbit gestalts; sustained by light and radiation, slow-moving, and broadly peaceful. See [Diona](../../Species/Diona.md) for a full species profile.

### Xenochimera

**Origin:** Unknown; biomass first encountered 2382 CE in the Scutum-Centaurus arm.

Colony organisms composed of microscopic hive cells that consume and mimic other life forms; sapience requires successful replication of a complex nervous system, and individuals are prone to feral regression under stress. See [Xenochimera](../../Species/Xenochimera.md) for a full species profile.

### Vox

**Origin:** Unknown; resident in outer-system arkships throughout known space.

Nomadic nitrogen-breathing reptilian bipeds living aboard ancient colossal vessels; a closed culture with a history of piracy and deeply limited contact with other spacefaring species. See [Vox](../../Species/Vox.md) for a full species profile.

### Protean

**Origin:** Unknown.

Nanoswarm-based synthetics capable of shapeshifting and self-regeneration; rare, tightly regulated, and under institutional scrutiny due to the instability of nanoswarm technology. See [Protean](../../Species/Protean.md) for a full species profile.

### Synthetics

Artificial constructs operating independently without behavioral laws, distinguished by three brain types: Cybernetic (organic brain in MMI hardware), Positronic (fully synthetic brain cube), and Drone (digital brain on computer card). See [Synthetics](../../Species/Synthetics.md) for a full species profile.

<details>
<summary>

### Shadekin

**Origin:** Unclassified.

No observational record on file.

</summary>

**Origin:** The Dark; documented in the Virgo-Erigone system.

Interdimensional beings of mammalian body plan capable of phase-shifting between physical space and a higher-energy plane; nearly immortal, empathic communicators, drawn to dimensional anomalies. See [Shadekin](../../Species/Shadekin.md) for a full species profile.

</details>

### Zaddat

**Origin:** Xohox, Vengeful Father system; now primarily in the Spacer Fleet.

Unathi client race requiring high-pressure life-support suits to survive in standard atmospheres; their homeworld's biosphere has largely collapsed, and most now live aboard habitat-ships in SolGov space. See [Zaddat](../../Species/Zaddat.md) for a full species profile.

### Alsvartr

**Origin:** United Hives; four Hiveworlds of undisclosed location.

Sapient xenomorph hybrids organized under a federal monarchy, bearing xenomorph biological traits alongside individual sapient consciousness, held together by the latent hivemind known as the Hivesong. See [Alsvartr](../../Species/Alsvartr.md) for a full species profile.

### Akula

**Origin:** Unknown aquatic homeworld; skrell client race.

Amphibious shark-featured humanoids discovered as tribal underwater sapients by the skrell; genome-modified to enable air-breathing, dependent on high humidity for sustained health. See [Akula](../../Species/Akula.md) for a full species profile.

### Nevrean

**Origin:** Tal, Vilous system.

Peaceful avian and dinosaur-like sapients with exceptional fine motor dexterity and a talent for precision instrument construction; share the Vilous system with sergals. See [Nevrean](../../Species/Nevrean.md) for a full species profile.

### Sergal

**Origin:** Vilous system.

Canine-featured bipedal sapients in two subspecies: aggressive Northern tundra-adapted sergals and more docile Southern urban sergals, with a long history of intersubspecies conflict including attempted genocide by both sides. See [Sergal](../../Species/Sergal.md) for a full species profile.

### Altevian

**Origin:** No homeworld; resident in colony-ship fleet.

Tall rodent-featured nomadic humanoids with no planetary home, sustained entirely aboard massive colony vessels; adapted to low-calorie diets and zero-gravity environments over generations of spaceflight. See [Altevian](../../Species/Altevian.md) for a full species profile.

### Rapala

**Origin:** Unknown; Unathi client race.

Coincidentally humanoid avian sapients with atmospheric flight capability and a dual chromosome sex-determination system; organized around hierarchical communal structures and operating as a client species under the Moghes Hegemony. See [Rapala](../../Species/Rapala.md) for a full species profile.

---

## Trans-Stellar Corporations

Trans-stellar corporations (TSCs) are private enterprises with operations spanning multiple star systems. Major TSCs maintain their own security forces, research installations, and logistics networks. Sol Central provides regulatory oversight in core systems; in frontier regions such as Vir, TSC influence often exceeds that of official government bodies.

The seven primary trans-stellars are: **NanoTrasen**, **Ward-Takahashi GMB**, **Gilthari Exports**, **Grayson Manufactories Ltd.**, **Aether Atmospherics and Recycling**, **Zeng-Hu Pharmaceuticals**, and **Hephaestus Industries**.

See [NanoTrasen](../../Organizations/NanoTrasen.md) for NanoTrasen-specific records.

---

## Governing Bodies

**Sol Central** is the primary governing body for humanity, focused on maintaining human interests and avoiding intervention in individual system governance except in cases of extreme emergency. Military presence is provided by the USDF. Sol Central's direct influence in frontier systems such as Vir is minimal.

**VGA (Vir Government Authority)** governs the Vir system as a full Sol Central member state. VGA operates as a unitary democratic state and maintains policies limiting TSC political influence within the system. Practical governance of the Kara subsystem is substantially delegated to NanoTrasen under the existing cooperation agreement.

---

## Military and Paramilitary Groups

**USDF (United Sol Defense Forces)** is the primary military presence in human-majority systems, based from Earth in Sol Central. NanoTrasen maintains lobbyists in Sol Central's government and operates in non-hostile alignment with the USDF. The USDF's predecessor organization, the United States Colonial Marines, is largely defunct.

**The Syndicate** is a paramilitary organization whose origins date to 2262. Active cells persist throughout the frontier despite periodic suppression. The Vir chapter is considered destroyed; however, independent operatives continue to surface with salvaged Syndicate equipment. The Syndicate maintains active hostility toward NanoTrasen and other major TSCs.

**The Ares Confederation** represents approximately twelve colony worlds on the fringe of human space. Anti-corporate and anti-Sol Central in orientation; relations with NanoTrasen are cold. No open hostilities have occurred.

**The 3rd Soviet Union** operates from Europa with support from scattered colonies and sympathizers. NanoTrasen maintains conditional neutrality with the Soviets through technology exchange agreements.

**NanoTrasen Defense Navy** is NanoTrasen's contracted military force, equipped with surplus USDF hardware and internally developed systems.

---

## Pirates and Independent Threats

Vir's distance from core systems and wealth of phoron deposits attract a consistent presence of pirates, smugglers, and opportunistic criminals. NanoTrasen's growing footprint in the system also draws periodic Syndicate activity.

The wise crew member does not give out trust freely.
